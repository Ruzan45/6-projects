...
function App() {
  const sortName = ['Все', 'Горы', 'Море', 'Архитектура', 'Города']
  const [items, setItems] = React.useState(data.collections)
  const [selectedSort, setSelectedSort] = React.useState(0);
  const [searchValue, setSearchValue] = React.useState('');
  React.useEffect(() => {
    const sorted = data.collections.filter(obj => { return obj.category === selectedSort })
    sorted.length > 0 ? setItems(sorted) : setItems(data.collections)
  }, [selectedSort])
  ...
  return (
    <div className="App">
      ...
      <div className="top">
        <ul className="tags">
          {sortName.map((obj, i) => (<li onClick={() => setSelectedSort(i)} className={selectedSort === i ? 'active' : ''}>{obj}</li>))}
        </ul>
      ...
      <div className="content">
        {items.filter(obj => { return obj.name.toLowerCase().includes(searchValue.toLowerCase()) })
          .map((obj, index) => (
